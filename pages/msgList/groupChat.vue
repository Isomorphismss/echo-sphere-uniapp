					...
					<block v-for="(m, index) in allMSGList" :key="index">
						...
						<view class="msg-line-right" v-if="m.msgType == MSGTYPE.WRODS && m.msgSource == MSGSOURCE.SENDER">
							<view class="every-msg-right">
								...
								<image src="../../static/images/default.png" class="face-icon"></image>
							</view>
						</view>
		...
		<view class="face-board-wrapper" :animation="animationData" style="height: 365px;">
			<scroll-view class="face-box" scroll-y="true">
				<view class="all-face" v-if="latestFaceList.length > 0">
					<text v-if="env != 'iso'" class="all-face-words">最近使用</text>
					<text v-if="env == 'iso'" class="all-face-words-iso">最近使用</text>
				</view>
				
				<view class="latest-face" v-if="latestFaceList.length > 0">
					<block v-for="(face, index) in latestFaceList" :key="'latest_face_' + index">
						<image v-if="env != 'iso'" :src="'../../static/images/gif/' + face + '.gif'" @click="enterFace(face)" style="" class='emoji-iamge-board'/>
						<image v-if="env == 'iso'" :src="'../../static/images/msface/' + face + '.png'"  style="" @click="enterFace(face)" class='ms-emoji'/>
					</block>
				</view>
				
				
				<view class="all-face">
					<text v-if="env != 'iso'" class="all-face-words">所有表情</text>
					<text v-if="env == 'iso'" class="all-face-words-iso">所有表情</text>
				</view>
				
				<view class="emoji-wrapper">
					<block v-for="(face, index) in faceArray" :key="'face_' + index">
						<image v-if="env != 'iso'" :src="'../../static/images/gif/' + face + '.gif'"  style="" @click="enterFace(face)" class='emoji-iamge-board'/>
						<image v-if="env == 'iso'" :src="'../../static/images/msface/' + face + '.png'"  style="" @click="enterFace(face)" class='ms-emoji'/>
		...
		methods: {
			...
			transform2Words() {
				var postData = {
					...
					cuid: 'Isomorphism', //用户唯一标识，用来区分用户，计算UV值。建议填写能区分用户的机器 MAC 地址或 IMEI 码，长度为60字符以内。
					token: this.token,
